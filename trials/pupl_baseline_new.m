function out = pupl_baseline_new(EYE, varargin)
...
if strcmp(args.correction, 'none') % Any baseline correction?
    ...
    args.event = EYE(1).event(1).name; % Should be 'beginning of recording'
    args.lims = {'1d' '1d'};
else
    if isempty(args.mapping)
        ...
        args.mapping = mappingOptions(...
            listdlg('PromptString', sprintf('Baseline-to-trial\nmapping'),...
