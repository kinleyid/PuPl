...
function out = pupl_tvar_map(EYE, varargin)

if nargin == 0
    out = @getargs;
else
    out = sub_tvar_map(EYE, varargin{:});
end
...
if isempty(args.old)
    all_tvars = pupl_tvar_getnames([EYE.event]);
    sel = listdlg(...
        'PromptString', 'Map which trial variables to numbers?',...
        'ListString', all_tvars,...
        'SelectionMode', 'single');
    if isempty(sel)
        return
    else
        args.old = all_tvars{sel};
    end
end
...
if isempty(args.new)
    new = inputdlg('Name of resulting numeric trial variable');
    if isempty(new)
        return
    else
        new = new{:};
...
function EYE = sub_tvar_map(EYE, varargin)
